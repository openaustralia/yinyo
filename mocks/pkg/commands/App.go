...
// ReportMemoryUsage provides a mock function with given fields: runID, memory, duration
func (_m *App) ReportMemoryUsage(runID string, memory uint64, duration time.Duration) error {
	ret := _m.Called(runID, memory, duration)

	var r0 error
	if rf, ok := ret.Get(0).(func(string, uint64, time.Duration) error); ok {
		r0 = rf(runID, memory, duration)
	} else {
		r0 = ret.Error(0)
	}
