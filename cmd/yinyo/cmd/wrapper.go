...
func streamLogs(run yinyoclient.Run, stage string, streamName string, stream io.ReadCloser, c chan error) {
	...
	for scanner.Scan() {
		run.CreateEvent(event.EventWrapper{Event: event.LogEvent{Stage: stage, Stream: streamName, Text: scanner.Text()}})
	}
	c <- scanner.Err()
}
...
func checkError(err error, run yinyoclient.Run, stage string, text string) {
	if err != nil {
		run.CreateEvent(event.EventWrapper{Event: event.LogEvent{Stage: "build", Stream: "interr", Text: "There was a problem getting the code"}})
		log.Fatal(err)
	}
}
...
var wrapperCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		err := run.CreateEvent(event.EventWrapper{Event: event.StartEvent{Stage: "build"}})
		...
		err = run.CreateEvent(event.EventWrapper{Event: event.FinishEvent{Stage: "build"}})
		checkError(err, run, "build", "Could not create event")
		...
		if exitData.Build.ExitCode == 0 {
			err = run.CreateEvent(event.EventWrapper{Event: event.StartEvent{Stage: "run"}})
			...
			err = run.CreateEvent(event.EventWrapper{Event: event.FinishEvent{Stage: "run"}})
			checkError(err, run, "run", "Could not create event")
		} else {
		...
		err = run.CreateEvent(event.EventWrapper{Event: event.LastEvent{}})
		checkError(err, run, "run", "Could not create event")
	},
}
